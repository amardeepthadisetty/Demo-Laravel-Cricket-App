...
<div class="row">
	<form class="form form-horizontal mar-top" action="{{route('templates.update', $template->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
		<input name="_method" type="hidden" value="POST">
		<input type="hidden" name="id" value="{{ $template->id }}">
		@csrf
		<div class="panel">
			<div class="panel-heading">
				<h3 class="panel-title">{{__('Template Information')}}</h3>
			</div>
				    ...
				    <div class="tab-content">
				        <div id="demo-stk-lft-tab-1" class="tab-pane fade active in">
							<div class="form-group">
	                            <label class="col-lg-2 control-label">{{__('Template Name')}}</label>
	                            <div class="col-lg-7">
	                                <input type="text" class="form-control" name="name" placeholder="{{__('Template Name')}}" value="{{$template->name}}"   required>
	                            </div>
	                        </div>
	                        ...
	                        <div class="form-group">
	                            <label class="col-lg-2 control-label">{{__('Template Type')}}</label>
	                            <div class="col-lg-7">
	                                <select class="form-control" name="template_type" required>
	                                	<option value="">Select Type</option>
	                                	<option value="template" @php if($template->template_type=="template"){ echo 'selected'; }  @endphp >Template</option>
	                                	<option value="layout" @php if($template->template_type=="layout"){ echo 'selected'; }  @endphp >Layout</option>
	                                </select>
	                            </div>
	                        </div>
	                        ...
	                        <div class="form-group">
								...
								<div class="col-lg-7">
									<input type="text" class="form-control" id="slug" name="slug" value="{{$template->slug}}" placeholder="{{__('SLUG')}}" required>
							...
							<div class="form-group">
								...
								<div class="col-lg-7">
									<input type="text" class="form-control" value="{{$template->model}}" name="model" placeholder="{{__('Model')}}" required>
								</div>
							</div>


							  {{--  <div class="form-group">
	                            <label class="col-lg-2 control-label">{{__('Template Categories')}}</label>
	                            <div class="col-lg-7">
	                                <select class="form-control template_categories " name="categories_add[]" multiple="multiple"  required>
	                                	<option value="">Select Categories</option>
	                                	@foreach($templateCategories as $prod)
		                                	<option value="{{ $prod->_id }}"  
		                                		@php 
		                                		if($template->categories){
		                                			foreach( $template->categories as $p){
		                                				if($prod->_id == $p){
		                                					echo 'selected';
		                                				}
		                                			}
	                        ...
	                        <div class="form-group">
							   ...
							   <div class="form-group">
	                            <label class="col-lg-2 control-label">{{__('Template Products')}}</label>
	                            <div class="col-lg-7">
	                        ...
	                        <div class="form-group">
								<label class="col-lg-2 control-label">{{__('Base Price')}}</label>
								<div class="col-lg-7">
									<input type="text" class="form-control" value="{{ $template->base_price }}" name="base_price" placeholder="{{__('Base Price')}}" required>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-lg-7">
									<input type="text" class="form-control" value="{{ $template->display_price }}" name="display_price" placeholder="{{__('Display Price')}}" required>
								</div>
							</div>
	                        ...
	                        <div class="form-group">
	                            ...
	                            <div class="col-lg-7">
	                                <input type="text" class="form-control" name="tags[]" id="tags" value="{{ $template->tags }}" placeholder="Type to add a tag" data-role="tagsinput">
	                            </div>
	                        </div>
				        ...
				        <div id="demo-stk-lft-tab-2" class="tab-pane fade">
								...
								<div class="col-lg-7">
									<div id="photos">
										@if(is_array($template->photos))
											@foreach ($template->photos as $key => $photo)
							...
							<div id="product_images">
								@if($template->products)
									@foreach( $template->products as $pids ) 
										<div class="form-group" id="id_div_{{ $pids }}">
											<label class="col-lg-2 control-label">{{ \App\Product::where('ref',$pids)->first()->name }}</label>
											<div class="col-lg-7" id="prod_id_{{ $pids }}">
												@php 
												...
												@endphp
												@if(  !empty( $obj->$pid_name )  )
													@foreach($obj->$pid_name as $photo)
														<div class="col-md-4 col-sm-4 col-xs-6">
															<div class="img-upload-preview">
																<img loading="lazy"  src="{{ asset( getImageURL($photo, 'thumbnail') ) }}" alt="" class="img-responsive">
																<input type="hidden" name="previous_product_photos_{{ $pids }}[]" value="{{ $photo }}">
																<button type="button" class="btn btn-danger close-btn remove-files"><i class="fa fa-times"></i></button>
															</div>
														</div>
						...
						<div id="demo-stk-lft-tab-4" class="tab-pane fade">
							<div class="form-group">
								...
								<div class="col-lg-7">
									<input type="text" class="form-control" name="meta_title" value="{{ $template->meta_title }}" placeholder="{{__('Meta Title')}}">
								</div>
							</div>
							<div class="form-group">
								<label class="col-lg-2 control-label">{{__('Description')}}</label>
								<div class="col-lg-7">
									<textarea name="meta_description" rows="8" class="form-control">{{ $template->meta_description }}</textarea>
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-lg-7">
									<div id="meta_photo">
										@if ($template->meta_img != null)
											<div class="col-md-4 col-sm-4 col-xs-6">
												<div class="img-upload-preview">
													<img loading="lazy"  src="{{ asset($template->meta_img) }}" alt="" class="img-responsive">
													<input type="hidden" name="previous_meta_img" value="{{ $template->meta_img }}">
													<button type="button" class="btn btn-danger close-btn remove-files"><i class="fa fa-times"></i></button>
												</div>
											</div>
										@endif
				        ...
				        <div id="demo-options" class="tab-pane fade">
				        	<div id="more_options"> 
				        	 @if(isset($template->additional_options))
						...
						<div id="demo-stk-lft-tab-7" class="tab-pane fade">
							<div class="form-group">
	                            ...
	                            <div class="col-lg-9">
	                                <textarea class="editor" name="small_description">{{$template->small_description}}</textarea>
	                            </div>
	                        </div>
	                        ...
	                        <div class="form-group">
	                            <label class="col-lg-2 control-label">{{__('Description')}}</label>
	                            <div class="col-lg-9">
	                                <textarea class="editor" name="description">{{$template->description}}</textarea>
	                            </div>
	                        </div>
				        </div>
				        ...
				        <div id="demo-stk-lft-tab-11" class="tab-pane fade">
							...
							<div class="form-group upload_limit_div">
										...
										<div class="col-lg-7">
											<input type="number" min="0" step="0.01" placeholder="{{__('Upload Limit')}}" name="upload_limit" class="form-control" value="{{ $template->more_settings->upload_settings->upload_limit }}" >
										</div>
							</div>
...
<script type="text/javascript">
	...
	function update_sku(){
		/*$.ajax({
		   type:"POST",
		   url:'{{ route('templates.sku_combination_edit') }}',
	...
	function update_option_sku(ref_row){
		...
		$.ajax({
		   type:"POST",
		   url:'{{ route('templates.more_options_edit') }}',
		   data:{
	...
	$('#slug').on('keyup', function(){
		...
		$.post('{{ route('templates.slug_check') }}',{_token:'{{ csrf_token() }}', slug_check:'edit', slug_value: this.value}, function(data){
